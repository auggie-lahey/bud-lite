...
---
[Token estimate: ~${f}]`,v=await Ja(l,b);return{question:e,answer:v,sources:i.map(h=>({author:h.author,content:h.content,date:h.created_at,score:h.score,hashtags:h.hashtags,event_id:h.event_id,kind:h.kind})),system_prompt:l,user_prompt:b}}async function no(e,{limit:t=10,pubkeys:n=[]}={}){const a=await qt(e),o=await Ft(a,{limit:t*2,pubkeys:n}),[i,r]=Ga(o,n,t),s=await Za(n),c=i.length?Ya(i):"No notes retrieved.";let l=An;r&&(l+=`
    ...
    <div class="mx-auto max-w-[1280px] px-4">
      ...
      <p class="mt-1 max-w-3xl text-sm text-white/75">Browse public archives from the Nostr network. Click any archive to explore its files.</p>
        ...
        <div class="col-span-full text-center py-12">
          <div class="text-zinc-400 text-sm">Loading archives from relays...</div>
        </div>`,c.style.display="none";return}for(const b of u){const v=b.picture?`<img src="${b.picture}" class="h-10 w-10 rounded-full object-cover" onerror="this.outerHTML='<div class=\\'ia-archive-avatar-fallback\\'>&#9786;</div>'">`:'<div class="ia-archive-avatar-fallback">&#9786;</div>',h=b.recentFiles.length>0?`<ul class="mt-2 space-y-0.5">${b.recentFiles.map(T=>`<li class="text-xs text-zinc-400 truncate">&#8226; ${te(T)}</li>`).join("")}</ul>`:'<div class="text-xs text-zinc-400 mt-1">No files yet</div>',x=document.createElement("a");x.href=`#/archive/${b.pubkey}`,x.className="block rounded-lg border border-[var(--color-ia-border)] bg-white p-4 hover:shadow-md transition-shadow no-underline",x.innerHTML=`
        <div class="flex items-center gap-3">
          ${v}
          <div class="min-w-0 flex-1">
        ...
        ${h}`,s.appendChild(x)}c.style.display=f?"block":"none"}return d(),a.push(t.subscribe("exploreArchives",d)),function(){a.forEach(f=>{try{f()}catch{}})}}function Nm(e,t){const n=document.getElementById("ia-main");return n.innerHTML="",bm(n),function(){n.innerHTML=""}}const En=300*1e3,we=$o({user:null,items:[],exploreArchives:[],exploreCacheTime:0,exploreHasMore:!0,followedPubkeys:[],blossomBase:"",cacheTime:0,settings:_e(),viewMode:localStorage.getItem("ia-view-mode")||"grid",listSort:localStorage.getItem("ia-list-sort")||"date-desc",currentRoute:null});document.documentElement.setAttribute("data-view",we.getState().viewMode);const Dm=document.getElementById("app");Dm.innerHTML=`
