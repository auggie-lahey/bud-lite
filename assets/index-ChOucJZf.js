...
print(cxr_client.get_image_embeddings_from_images([img]))`],vd=e=>{let t,n,a;return t="<ENCODER>",n="<NUMBER_OF_FEATURES>",a="<OUT_CHANNELS>",e.id==="depth-anything/Depth-Anything-V2-Small"?(t="vits",n="64",a="[48, 96, 192, 384]"):e.id==="depth-anything/Depth-Anything-V2-Base"?(t="vitb",n="128",a="[96, 192, 384, 768]"):e.id==="depth-anything/Depth-Anything-V2-Large"&&(t="vitl",n="256",a="[256, 512, 1024, 1024"),[`
...
)`],Sd="pip install -U diffusers transformers accelerate",Fa="Astronaut in a jungle, cold color palette, muted colors, detailed, 8k",Ha="Turn this cat into a dog",xn="A man with short gray hair plays a red electric guitar.",Id=e=>[`import torch
...
`,oa={CausalLM:Gd,TextToImage:ep,TextClassifier:tp,ImageClassifier:np},ap=(e,t)=>`
import keras_hub
...
`,ip=e=>{var o,i;const t=e.id,n=((i=(o=e.config)==null?void 0:o.keras_hub)==null?void 0:i.tasks)??[],a=[];for(const[r,s]of Object.entries(oa))n.includes(r)&&a.push(s(t));for(const r of n)Object.keys(oa).includes(r)||a.push(ap(r,t));return a.push(op(t)),a},rp=e=>[`# !pip install kernels
...
torchaudio.save("output1.wav", torch.from_numpy(wavs[0]), 24000)`],ia=e=>{const t=e.tags.find(o=>o.match(/^yolov\d+$/)),n=t?`YOLOv${t.slice(4)}`:"YOLOvXX";return[(t?"":`# Couldn't find a valid YOLO version tag.
...
model = BiRefNet.from_pretrained("${e.id}")`],vu=()=>[`from supertonic import TTS
...
{% if billTo %} \\
    -H 'X-HF-Bill-To: {{ billTo }}'
{% endif %}`}}},Ka=(e,t,n)=>{var o,i;const a=(i=(o=Ju[e])==null?void 0:o[t])==null?void 0:i[n];if(!a)throw new Error(`Template not found: ${e}/${t}/${n}`);return r=>new El(a).render({...r})};Ka("python","huggingface_hub","importInferenceClient");Ka("python","requests","importRequests");let Wa=null;async function Xa(){try{const e=await fetch("/bud-lite/qdrant-config.json");e.ok&&(Wa=await e.json())}catch{}}function _n(){return Wa||{}}function Bt(){const e=_e(),t=_n();let n=(e.qdrantUrl||t.url||"").replace(/\/$/,"");return n&&n.includes("cloud.qdrant.io")&&!n.match(/:\d+/)&&(n+=":6333"),n&&typeof window<"u"&&n.includes("cloud.qdrant.io")&&(n="https://corsproxy.io/?"+encodeURIComponent(n)),{url:n,apiKey:e.qdrantApiKey||t.readOnlyKey||"",collection:e.qdrantCollection||"nostr_rag"}}function Yu(){const e=_e();return{apiKey:e.llmApiKey||"",baseUrl:(e.llmBaseUrl||"https://api.z.ai/api/paas/v4").replace(/\/$/,""),model:e.llmModel||"GLM-5.1"}}function Zu(){const e=_e(),t=_n();return{apiKey:e.hfApiKey||(t._h?t._h.split("").reverse().join(""):""),model:"mixedbread-ai/mxbai-embed-large-v1"}}async function qt(e){return(await Qa([e]))[0]}async function Qa(e){const{apiKey:t,model:n}=Zu();if(!t)throw new Error("HuggingFace API key required for search.");return await new Ls(t).featureExtraction({model:n,inputs:e})}async function Gu(e){var i;const{url:t,collection:n}=Bt();if(!t)return{};const a=kn(),o={};for(const r of e)try{const s=await fetch(`${t}/collections/${n}/points/count`,{method:"POST",headers:a,body:JSON.stringify({filter:{must:[{key:"pubkey",match:{value:r}}]},exact:!1})});if(s.ok){const c=await s.json();o[r]=((i=c.result)==null?void 0:i.count)||0}}catch{}return o}function kn(){const{apiKey:e}=Bt(),t={"Content-Type":"application/json"};return e&&(t["api-key"]=e),t}async function Ft(e,{limit:t=15,pubkeys:n=[]}={}){const{url:a,collection:o}=Bt();if(!a)throw new Error("Qdrant URL not configured");const i={vector:e,limit:t,with_payload:!0};n.length&&(i.filter={must:[{key:"pubkey",match:{any:n}}]});const r=await fetch(`${a}/collections/${o}/points/search`,{method:"POST",headers:kn(),body:JSON.stringify(i)});if(!r.ok){const c=await r.text().catch(()=>"");throw new Error(`Qdrant search failed (${r.status}): ${c}`)}return((await r.json()).result||[]).map(c=>({score:c.score,author:(c.payload||{}).author_label||((c.payload||{}).pubkey||"").slice(0,8),pubkey:(c.payload||{}).pubkey||"",content:(c.payload||{}).content||"",created_at:(c.payload||{}).created_at||0,hashtags:(c.payload||{}).hashtags||[],event_id:(c.payload||{}).event_id||"",kind:(c.payload||{}).kind,source_type:(c.payload||{}).source_type||""}))}async function em(){const{url:e,collection:t}=Bt();if(!e)return null;try{const n=await fetch(`${e}/collections/${t}`,{headers:kn()});return n.ok?(await n.json()).result:null}catch{return null}}async function Ja(e,t){const{apiKey:n,baseUrl:a,model:o}=Yu();if(!n)throw new Error("LLM API key not configured");const i=a.includes("anthropic.com");if(a.includes("z.ai"),i){const l=await fetch(`${a}/v1/messages`,{method:"POST",headers:{"Content-Type":"application/json","x-api-key":n,"anthropic-version":"2023-06-01"},body:JSON.stringify({model:o,max_tokens:1024,system:e,messages:[{role:"user",content:t}]})});if(!l.ok){const u=await l.text().catch(()=>"");throw new Error(`LLM call failed (${l.status}): ${u}`)}return(await l.json()).content[0].text}const r=a.endsWith("/v4")||a.endsWith("/v4/")?`${a}/chat/completions`:`${a}/v1/chat/completions`,s=await fetch(r,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${n}`},body:JSON.stringify({model:o,max_tokens:1024,messages:[{role:"system",content:e},{role:"user",content:t}]})});if(!s.ok){const l=await s.text().catch(()=>"");throw new Error(`LLM call failed (${s.status}): ${l}`)}return(await s.json()).choices[0].message.content}const An=`The notes below were retrieved via semantic search (vector similarity) against a database of Nostr posts. They are ranked by relevance to the user's query.
...
`)}async function Za(e){try{const t=await fetch("/soul-hints.json");if(!t.ok)return"";const n=await t.json(),a=n.hints||{},o=n.micros||{},i=["=== Group Members (compact profiles) ==="];for(const[r,s]of Object.entries(a)){if(e.length&&!e.includes(r))continue;const c=s.split(`
`)[0].replace(/\*\*/g,"").trim()||r.slice(0,8);i.push(`**${c}**: ${s}`)}return i.join(`
`)}catch{return""}}function Ga(e,t,n){const a=new Set(t||[]),o=e.filter(d=>d.score>=.3),i=new Set(o.map(d=>d.pubkey)),r=[];if(a.size){for(const d of a)if(!i.has(d)){let u=null;for(const f of e)f.pubkey===d&&(!u||f.score>u.score)&&(u=f);u&&r.push(u)}}const s=new Set,c=[];for(const d of[...o,...r]){const u=`${d.pubkey}:${d.content.slice(0,50)}`;s.has(u)||(s.add(u),c.push(d))}const l=o.length<e.length/2;return[c.slice(0,n),l]}async function eo(e,{limit:t=10,pubkeys:n=[]}={}){const a=await qt(e),o=await Ft(a,{limit:t,pubkeys:n});return{query:e,count:o.length,results:o}}async function to(e,{limit:t=10,pubkeys:n=[]}={}){const a=await qt(e),o=await Ft(a,{limit:t*2,pubkeys:n}),[i,r]=Ga(o,n,t);if(!i.length)return{question:e,answer:"No relevant notes found for your question.",sources:[]};const s=await Za(n),c=Ya(i);let l=An;r&&(l+=`
...
${c}`),d.push(`Based on the above context, answer this question: ${e}`);const u=d.join(`

---

`),f=Math.floor((l.length+u.length)/4),b=u+`

---
[Token estimate: ~${f}]`,w=await Ja(l,b);return{question:e,answer:w,sources:i.map(h=>({author:h.author,content:h.content,date:h.created_at,score:h.score,hashtags:h.hashtags,event_id:h.event_id,kind:h.kind})),system_prompt:l,user_prompt:b}}async function no(e,{limit:t=10,pubkeys:n=[]}={}){const a=await qt(e),o=await Ft(a,{limit:t*2,pubkeys:n}),[i,r]=Ga(o,n,t),s=await Za(n),c=i.length?Ya(i):"No notes retrieved.";let l=An;r&&(l+=`
...
`);return{system_prompt:l,user_prompt:u,notes_count:i.length,low_confidence:r,scores:i.map(f=>Math.round(f.score*1e3)/1e3)}}async function ao(){return An}async function oo(){try{const e=await fetch("/bud-lite/soul-hints.json");if(!e.ok)return[];const t=await e.json(),n=t.micros||{},a=t.labels||{},o=t.pictures||{},i=Object.keys(n);return i.length?i.map(r=>({pubkey:r,name:a[r]||r.slice(0,8),label:a[r]||"",picture:o[r]||"",micro:n[r]||""})):[]}catch{return[]}}function Ct(){const e=_e(),t=_n();return{hf:!!(e.hfApiKey||(t._h?t._h.split("").reverse().join(""):"")),llm:!!e.llmApiKey,groq:!!e.groqApiKey,gemini:!!e.geminiApiKey,qdrant:!!(e.qdrantUrl||t.url)}}const tm=Object.freeze(Object.defineProperty({__proto__:null,askLLM:Ja,countNotesPerPubkey:Gu,embedText:qt,embedTexts:Qa,getKeyStatus:Ct,getQdrantInfo:em,loadDeployedConfig:Xa,ragAsk:to,ragGetPubkeys:oo,ragGetSystemPrompt:ao,ragPreview:no,ragSearch:eo,searchQdrant:Ft},Symbol.toStringTag,{value:"Module"}));function te(e){return(e||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function bt(e){return!e&&e!==0?"":e<1024?e+" B":e<1048576?(e/1024).toFixed(1)+" KB":e<1073741824?(e/1048576).toFixed(1)+" MB":(e/1073741824).toFixed(1)+" GB"}const Je={video:"🎬",audio:"🎵",images:"🖼",software:"📦",web:"🌐",texts:"📄"};function Fe(e){const t=(e||"").toLowerCase();return t.startsWith("video/")?"video":t.startsWith("audio/")?"audio":t.startsWith("image/")?"images":t==="text/html"?"web":/zip|tar|gz|xz|7z|rar/.test(t)?"software":"texts"}function Pe(e){const t=document.createElement("div");t.className="ia-toast",t.textContent=e,document.body.appendChild(t),setTimeout(()=>t.remove(),1600)}function nm(e){const t=[],n=document.getElementById("ia-settings-fab"),a=document.getElementById("ia-settings-overlay"),o=document.getElementById("ia-settings-close"),i=document.getElementById("ia-settings-form");if(!n||!a||!i)return console.warn("[settings-modal] Required DOM elements not found"),()=>{};function r(){const d=_e();document.getElementById("set-archiver-npub").value=d.archiverNpub||"",document.getElementById("set-blossom-url").value=d.blossomUrl||"",document.getElementById("set-blossom-mirror").value=d.blossomMirror||"",document.getElementById("set-relays").value=d.relays||"",document.getElementById("set-manifest-dtag").value=d.manifestDtag||"archive",document.getElementById("set-rag-url").value=d.ragBackendUrl||"",document.getElementById("set-qdrant-url").value=d.qdrantUrl||"",document.getElementById("set-qdrant-key").value=d.qdrantApiKey||"",document.getElementById("set-qdrant-collection").value=d.qdrantCollection||"nostr-rag",document.getElementById("set-hf-key").value=d.hfApiKey||"",document.getElementById("set-llm-key").value=d.llmApiKey||"",document.getElementById("set-llm-base-url").value=d.llmBaseUrl||"https://api.anthropic.com",document.getElementById("set-llm-model").value=d.llmModel||"claude-sonnet-4-5-20250514",document.getElementById("set-groq-key").value=d.groqApiKey||"",document.getElementById("set-gemini-key").value=d.geminiApiKey||"",a.classList.remove("hidden")}function s(){a.classList.add("hidden")}function c(d){d.target===a&&a.classList.add("hidden")}function l(d){d.preventDefault(),fa({archiverNpub:document.getElementById("set-archiver-npub").value.trim(),blossomUrl:document.getElementById("set-blossom-url").value.trim(),blossomMirror:document.getElementById("set-blossom-mirror").value.trim(),relays:document.getElementById("set-relays").value.trim(),manifestDtag:document.getElementById("set-manifest-dtag").value.trim()||"archive",ragBackendUrl:document.getElementById("set-rag-url").value.trim(),qdrantUrl:document.getElementById("set-qdrant-url").value.trim(),qdrantApiKey:document.getElementById("set-qdrant-key").value.trim(),qdrantCollection:document.getElementById("set-qdrant-collection").value.trim()||"nostr-rag",hfApiKey:document.getElementById("set-hf-key").value.trim(),llmApiKey:document.getElementById("set-llm-key").value.trim(),llmBaseUrl:document.getElementById("set-llm-base-url").value.trim()||"https://api.anthropic.com",llmModel:document.getElementById("set-llm-model").value.trim()||"claude-sonnet-4-5-20250514",groqApiKey:document.getElementById("set-groq-key").value.trim(),geminiApiKey:document.getElementById("set-gemini-key").value.trim()}),a.classList.add("hidden"),Pe("Settings saved")}return n.addEventListener("click",r),o.addEventListener("click",s),a.addEventListener("click",c),i.addEventListener("submit",l),t.push(()=>{n.removeEventListener("click",r),o.removeEventListener("click",s),a.removeEventListener("click",c),i.removeEventListener("submit",l)}),()=>t.forEach(d=>{try{d()}catch{}})}const am="ia-sync-handles",wt="handles";function io(){return new Promise((e,t)=>{const n=indexedDB.open(am,1);n.onupgradeneeded=()=>n.result.createObjectStore(wt),n.onsuccess=()=>e(n.result),n.onerror=()=>t(n.error)})}async function om(e){const t=await io();return new Promise((n,a)=>{const i=t.transaction(wt,"readonly").objectStore(wt).get(e);i.onsuccess=()=>n(i.result),i.onerror=()=>a(i.error)})}async function im(e,t){const n=await io();return new Promise((a,o)=>{const i=n.transaction(wt,"readwrite");i.objectStore(wt).put(t,e),i.oncomplete=()=>a(),i.onerror=()=>o(i.error)})}async function ut(e,t,n,a){const o=await Rt(t),i={kind:24242,content:"nostr-archive: upload",created_at:Math.floor(Date.now()/1e3),tags:[["t","upload"],["x",o],["expiration",String(Math.floor(Date.now()/1e3)+600)]]};if(!window.nostr)throw new Error("No NIP-07 signer found. Install nos2x or Alby.");const r=await window.nostr.signEvent(i),s=JSON.stringify(r),c="Nostr "+btoa(s),l=await fetch(e.replace(/\/$/,"")+"/upload",{method:"PUT",headers:{Authorization:c,"Content-Type":a||"application/octet-stream"},body:t});if(!l.ok)throw new Error(`Blossom upload failed: ${l.status} ${await l.text()}`);const d=await l.json();return{sha256:o,size:t.byteLength,...d}}const rm=[["application/pdf","texts"],["text/","texts"],["image/","images"],["audio/","audio"],["video/","video"],["application/zip","software"],["application/x-tar","software"],["application/gzip","software"]];function sm(e){for(const[t,n]of rm)if(e.startsWith(t))return n;return"other"}function mt(){return _e().manifestDtag||"archive"}function lm(e){const t=[],n=document.getElementById("ingest-overlay"),a=document.getElementById("ingest-close"),o=document.getElementById("ingest-fab"),i=document.getElementById("ingest-form");if(!n||!i)return console.warn("[ingest-panel] Required DOM elements not found"),()=>{};let r="url",s=null,c=null,l=null,d=null;const u=document.getElementById("ingest-file-drop"),f=document.getElementById("ingest-file-input"),b=document.getElementById("ingest-file-name"),w=document.getElementById("ingest-sync-drop"),h=document.getElementById("ingest-folder-input"),x=document.getElementById("ingest-sync-info"),T=document.getElementById("ingest-sync-stored"),C=document.getElementById("ingest-sync-diff"),$=document.getElementById("ingest-sync-btn"),A=document.getElementById("ingest-sync-force"),P=document.getElementById("ingest-sync-force-label"),R=typeof window.showDirectoryPicker=="function";function B(){n.classList.remove("hidden")}function W(){n.classList.add("hidden")}function v(p){p.target===n&&n.classList.add("hidden")}o&&o.addEventListener("click",B),a.addEventListener("click",W),n.addEventListener("click",v),t.push(()=>{o&&o.removeEventListener("click",B),a.removeEventListener("click",W),n.removeEventListener("click",v)});function L(p){r=p,document.querySelectorAll(".ingest-tab").forEach(k=>k.classList.toggle("active",k.dataset.tab===p)),document.querySelectorAll(".ingest-panel").forEach(k=>k.classList.toggle("active",k.id===`ingest-panel-${p}`));const m=p==="sync";document.querySelectorAll("#ingest-form > .ingest-row").forEach(k=>k.style.display=m?"none":""),document.querySelectorAll("#ingest-form > .checkbox-label").forEach(k=>k.style.display=m?"none":"");const g=document.getElementById("ingest-btn");g&&(g.style.display=m?"none":"")}document.querySelectorAll(".ingest-tab").forEach(p=>{const m=()=>L(p.dataset.tab);p.addEventListener("click",m),t.push(()=>p.removeEventListener("click",m))});function U(p){L(p.detail.tab)}document.addEventListener("ia-switch-tab",U),t.push(()=>document.removeEventListener("ia-switch-tab",U));function N(p){s=p,b.textContent=`${p.name} (${(p.size/1048576).toFixed(1)} MB)`}if(u){const p=()=>f.click(),m=E=>{E.preventDefault(),u.classList.add("dragover")},g=()=>u.classList.remove("dragover"),k=E=>{E.preventDefault(),u.classList.remove("dragover"),E.dataTransfer.files.length&&N(E.dataTransfer.files[0])};u.addEventListener("click",p),u.addEventListener("dragover",m),u.addEventListener("dragleave",g),u.addEventListener("drop",k),t.push(()=>{u.removeEventListener("click",p),u.removeEventListener("dragover",m),u.removeEventListener("dragleave",g),u.removeEventListener("drop",k)})}if(f){const p=()=>{f.files.length&&N(f.files[0])};f.addEventListener("change",p),t.push(()=>f.removeEventListener("change",p))}async function H(p){const m=[];async function g(k,E){for await(const X of k.values())if(X.kind==="file"){const re=await X.getFile(),Ce=new File([re],re.name,{type:re.type,lastModified:re.lastModified});Object.defineProperty(Ce,"webkitRelativePath",{value:E+re.name,writable:!1}),Object.defineProperty(Ce,"size",{value:re.size}),m.push(Ce)}else X.kind==="directory"&&await g(X,E+X.name+"/")}return await g(p,p.name+"/"),m}async function ee(p){var k;if(!p.length)return;if(!window.nostr){Pe("NIP-07 extension required");return}d=p;const m=((k=p[0].webkitRelativePath)==null?void 0:k.split("/")[0])||"folder",g=p.reduce((E,X)=>E+X.size,0);x.textContent=`${m} — ${p.length} files — ${bt(g)}`,C.textContent="Computing diff...",$.style.display="none",P.style.display="inline-flex";try{const E=[];for(const F of p){const ne=await F.arrayBuffer(),Ie=await Rt(ne),Ee="/"+F.webkitRelativePath.split("/").slice(1).join("/");E.push({file:F,sha256:Ie,relativePath:Ee,mime:F.type||"application/octet-stream",size:F.size})}const X=await window.nostr.getPublicKey(),Ce=_e().relays.split(",").map(F=>F.trim()).filter(Boolean),Oe=await yt(X,mt(),Ce),et=Oe?ot(Oe.content):{},pe=A&&A.checked,he=[],se=[],de=[];for(const F of E)if(pe)et[F.relativePath]?se.push(F):he.push(F);else{const ne=et[F.relativePath];ne?ne.sha256!==F.sha256?se.push(F):de.push(F):he.push(F)}c={toUpload:[...he,...se],unchanged:de};let J="";if(pe&&se.length>0&&(J+=`<div style="color:#fbbf24;margin-bottom:0.3em">Force re-sync: ${se.length} files will be re-uploaded</div>`),he.length>0){J+=`<div style="color:#6ee7b7;margin-bottom:0.3em">+ ${he.length} new</div>`;for(const F of he.slice(0,8))J+=`<div style="color:#6ee7b7;padding-left:1em">${te(F.relativePath)} <span style="color:#555">${bt(F.size)}</span></div>`;he.length>8&&(J+=`<div style="color:#555;padding-left:1em">+${he.length-8} more</div>`)}if(se.length>0&&!pe){J+=`<div style="color:#fbbf24;margin:0.5em 0 0.3em">~ ${se.length} changed</div>`;for(const F of se.slice(0,5))J+=`<div style="color:#fbbf24;padding-left:1em">${te(F.relativePath)}</div>`}de.length>0&&(J+=`<div style="color:#555;margin-top:0.5em">= ${de.length} unchanged (skipped)</div>`),c.toUpload.length===0?(J='<div style="color:#6ee7b7">All files up to date. Nothing to sync.</div>',C.innerHTML=J,P.style.display="inline-flex"):(C.innerHTML=J,$.style.display="inline-block",P.style.display="inline-flex",$.textContent=`Sync ${c.toUpload.length} files`),localStorage.setItem("ia-sync-folder",m)}catch(E){C.innerHTML=`<div style="color:#f87171">Error: ${E.message}</div>`}}if((async()=>{if(R)try{const p=await om("sync-dir-handle");p&&await p.queryPermission({mode:"read"})==="granted"&&(l=p,T.style.display="block",T.textContent=`Remembered folder: ${p.name} (click to re-sync)`,w.querySelector("div:last-child").textContent=p.name)}catch{}})(),w){const p=async()=>{if(R){if(l)try{if(await l.requestPermission({mode:"read"})==="granted"){C.textContent="Reading folder...";const g=await H(l);await ee(g);return}}catch{}try{const m=await window.showDirectoryPicker({mode:"read"});l=m,await im("sync-dir-handle",m),T.style.display="block",T.textContent=`Folder saved: ${m.name} (remembered for next visit)`,w.querySelector("div:last-child").textContent=m.name;const g=await H(m);await ee(g)}catch(m){m.name!=="AbortError"&&console.warn("Directory picker error:",m)}}else h.click()};w.addEventListener("click",p),t.push(()=>w.removeEventListener("click",p))}if(T){const p=async()=>{if(l)try{if(await l.requestPermission({mode:"read"})!=="granted"){Pe("Permission denied for folder access");return}C.textContent="Reading folder...";const g=await H(l);await ee(g)}catch(m){C.innerHTML=`<div style="color:#f87171">Error: ${m.message}</div>`}};T.addEventListener("click",p),T.style.cursor="pointer",t.push(()=>T.removeEventListener("click",p))}if(h){const p=()=>{h.files.length>0&&ee(Array.from(h.files))};h.addEventListener("change",p),t.push(()=>h.removeEventListener("change",p))}if(A){const p=()=>{d&&ee(d)};A.addEventListener("change",p),t.push(()=>A.removeEventListener("change",p))}if($){const p=async()=>{if(!(!c||!window.nostr)){$.disabled=!0,$.textContent="Syncing...";try{const m=_e(),g=m.relays.split(",").map(se=>se.trim()).filter(Boolean),k=await window.nostr.getPublicKey(),E=await yt(k,mt(),g),X=E?ot(E.content):{};for(let se=0;se<c.toUpload.length;se++){const de=c.toUpload[se],J=de.relativePath.split("/").pop();C.innerHTML+=`<div style="color:#fbbf24">${se+1}/${c.toUpload.length} ${te(J)}...</div>`,C.scrollTop=C.scrollHeight;const F=await de.file.arrayBuffer();if(await ut(m.blossomUrl,new Uint8Array(F),J,de.mime),m.blossomMirror)try{await ut(m.blossomMirror,new Uint8Array(F),J,de.mime)}catch{}console.log(`[sync] uploaded ${se+1}/${c.toUpload.length}: ${de.relativePath} sha=${de.sha256.slice(0,12)}`),X[de.relativePath]={title:J,sha256:de.sha256,mime:de.mime,size:de.size,added:Math.floor(Date.now()/1e3)}}const re=rn(X),Ce=await an(re),Oe=[...re,["d",mt()],["x",Ce,"aggregate"],["title","Personal Archive"],["server",m.blossomUrl]];m.blossomMirror&&Oe.push(["server",m.blossomMirror]);const et=await window.nostr.signEvent({kind:35128,content:on(X),created_at:Math.floor(Date.now()/1e3),tags:Oe});console.log("[sync] Signed manifest event:",et.id,"entries:",Object.keys(X).length);const pe=await at(et,g),he=pe.filter(se=>se.ok).length;C.innerHTML+=`<div style="color:#6ee7b7;margin-top:0.5em">Done! ${c.toUpload.length} files synced. Manifest published to ${he}/${pe.length} relays.</div>`,$.textContent="Synced!",c=null,A&&(A.checked=!1)}catch(m){C.innerHTML+=`<div style="color:#f87171">Error: ${m.message}</div>`,$.disabled=!1,$.textContent="Retry Sync"}}};$.addEventListener("click",p),t.push(()=>$.removeEventListener("click",p))}const be=localStorage.getItem("ia-last-folder");if(be){const p=document.getElementById("ingest-folder");p&&(p.value=be)}function ie(p,m,g){const k=document.getElementById("ingest-steps"),E=m?"✓":"✗",X=m?"#6ee7b7":"#f87171";k.innerHTML+=`<div class="ingest-step" style="color:${X}">${E} ${g}</div>`}const ce=async p=>{p.preventDefault();const m=document.getElementById("ingest-btn"),g=document.getElementById("ingest-progress"),k=document.getElementById("ingest-steps"),E=document.getElementById("ingest-current"),X=document.getElementById("ingest-title").value.trim(),re=document.getElementById("ingest-folder").value.trim(),Ce=(document.getElementById("ingest-topics").value.trim()||"").split(",").map(pe=>pe.trim()).filter(Boolean),Oe=document.getElementById("ingest-source-kind").value;if(re&&localStorage.setItem("ia-last-folder",re),r==="url"){const pe=document.getElementById("ingest-url").value.trim();if(!pe)return;if(!window.nostr||!window.nostr.signEvent){Pe("Install a Nostr signer extension (nos2x, Alby, etc) to flag URLs");return}const he=_e();m.disabled=!0,m.textContent="Flagging...",E.textContent="",g.style.display="block",k.innerHTML="";try{const se=[`**URL:** ${pe}`,X?`**Title:** ${X}`:"",re?`**Folder:** ${re}`:"",Oe?`**Source kind:** ${Oe}`:"",Ce.length?`**Topics:** ${Ce.join(", ")}`:""].filter(Boolean).join(`
`),de=X||`[archive] ${pe}`,J=Math.floor(Date.now()/1e3),F=[["subject",de],["t","archive"],["r",pe],...Ce.map($e=>["t",$e]),["alt",`Archive flag: ${de}`]];if(he.archiverNpub){const $e=Uo(he.archiverNpub);$e&&F.push(["p",$e])}const ne={kind:1621,content:se,tags:F,created_at:J},Ie=await window.nostr.signEvent(ne);ie("sign",!0,"Signed issue event");const ke=he.relays.split(",").map($e=>$e.trim()).filter(Boolean),Ee=await at(Ie,ke),ct=Ee.filter($e=>$e.ok).length;if(ct>0){const $e=Ee.filter(Be=>Be.ok).map(Be=>{try{return new URL(Be.url).hostname}catch{return Be.url}}).join(", ");ie("publish",!0,`Published to ${ct}/${Ee.length} relays (${$e})`),ie("event",!0,`Event ID: ${Ie.id}`),E.innerHTML=`Flagged for archiving! Event ID: <code style="color:#6ee7b7">${Ie.id.slice(0,12)}...</code>`}else{const $e=Ee.map(Be=>{try{return`${new URL(Be.url).hostname}: ${Be.message}`}catch{return`${Be.url}: ${Be.message}`}}).join("; ");ie("publish",!1,`Failed: ${$e}`),ie("event",!0,`Event ID: ${Ie.id}`),E.textContent="Failed to publish to any relay"}m.disabled=!1,m.textContent="Flag this URL"}catch(se){E.textContent=`Error: ${se.message}`,m.disabled=!1,m.textContent="Flag this URL"}}else{let J=function(F,ne,Ie){const ke=de[F];if(!ke)return;const Ee=ke.querySelector(".ingest-step-icon"),ct=ke.querySelector(".ingest-step-detail");ne==="running"?(ke.className="ingest-step ingest-step-running",Ee.textContent="●"):ne==="done"?(ke.className="ingest-step ingest-step-done",Ee.textContent="✓"):ne==="error"&&(ke.className="ingest-step ingest-step-error",Ee.textContent="✗"),Ie&&(ct.textContent=Ie),E.textContent=`${F}: ${ne}`};var et=J;if(!s){Pe("Select a file first");return}const pe=_e(),he=pe.relays.split(",").map(F=>F.trim()).filter(Boolean);m.disabled=!0,m.textContent="Uploading...",g.style.display="block",k.innerHTML="",E.textContent="";const se=[{key:"hash",label:"Compute hash"},{key:"upload",label:"Upload to Blossom"},{key:"manifest",label:"Build manifest"},{key:"sign",label:"Sign events"},{key:"publish",label:"Publish to relays"},{key:"nsite",label:"Update archive manifest"}],de={};for(const F of se){const ne=document.createElement("div");ne.className="ingest-step ingest-step-pending",ne.innerHTML=`<span class="ingest-step-icon">○</span> <span class="ingest-step-label">${F.label}</span> <span class="ingest-step-detail"></span>`,k.appendChild(ne),de[F.key]=ne}try{J("hash","running");const F=await s.arrayBuffer(),ne=await Rt(F),Ie=s.type||"application/octet-stream",ke=s.name,Ee=s.size;J("hash","done",ne),J("upload","running");const ct=await ut(pe.blossomUrl,new Uint8Array(F),ke,Ie);if(J("upload","done",bt(Ee)),pe.blossomMirror)try{await ut(pe.blossomMirror,new Uint8Array(F),ke,Ie)}catch{}J("manifest","running");const ho={version:"1.0",type:"archive-bundle",title:X||ke,source:{kind:Oe||"webpage",url:""},blobs:[{x:ne,role:"source",filename:ke,m:Ie,size:Ee}]},go=new TextEncoder().encode(JSON.stringify(ho,null,2)+`
`),It=await ut(pe.blossomUrl,go,"manifest.json","application/json");J("manifest","done",It.sha256),J("sign","running");const yo={kind:1115,content:"",created_at:Math.floor(Date.now()/1e3),tags:[["x",ne],["x",It.sha256],["primary",It.sha256,String(It.size)]]},Et=await window.nostr.signEvent(yo);J("sign","done",`Bridge: ${Et.id}`);const Wt=[["e",Et.id],["title",X||ke]],Xt=re||sm(Ie);Xt&&Wt.push(["t",Xt]);const Cn=document.getElementById("ingest-topics");if(Cn&&Cn.value.trim())for(const Le of Ce)Wt.push(["t",Le]);const bo={kind:1116,content:"",created_at:Math.floor(Date.now()/1e3),tags:Wt},Qt=await window.nostr.signEvent(bo);J("publish","running");const wo=await at(Et,he),vo=await at(Qt,he),$n=[...wo,...vo],xo=$n.filter(Le=>Le.ok).length,_o=$n.filter(Le=>Le.ok).map(Le=>{try{return new URL(Le.url).hostname}catch{return Le.url}}),ko=[...new Set(_o)];J("publish","done",`${xo}/${he.length*2} ok (${ko.join(", ")})`),ie("event",!0,`Index: ${Qt.id}`),J("nsite","running");try{const Le=await window.nostr.getPublicKey(),Rn=await yt(Le,mt(),he),Jt=Rn?ot(Rn.content):{},Ao=`/${Xt}/${ke}`;Jt[Ao]={title:X||ke,added:Math.floor(Date.now()/1e3),source:{kind:Oe||"webpage",url:""},topics:Ce,bridge_event_id:Et.id,index_event_id:Qt.id,sha256:ne,mime:Ie,size:Ee};const Un=rn(Jt),To=await an(Un),Mn=[...Un,["d",mt()],["x",To,"aggregate"],["title","Personal Archive"],["server",pe.blossomUrl]];pe.blossomMirror&&Mn.push(["server",pe.blossomMirror]);const So={kind:35128,content:on(Jt),created_at:Math.floor(Date.now()/1e3),tags:Mn},Io=await window.nostr.signEvent(So),Nn=await at(Io,he),Eo=Nn.filter(Lo=>Lo.ok).length;J("nsite","done",`Manifest published to ${Eo}/${Nn.length} relays`)}catch(Le){console.warn("Nsite manifest publish failed:",Le),J("nsite","error",Le.message)}E.textContent="Upload complete! Close this panel and refresh to see the new item.",m.disabled=!1,m.textContent="Archive this URL"}catch(F){E.textContent=`Error: ${F.message}`,m.disabled=!1,m.textContent="Archive this URL";for(const ne of Object.keys(de))de[ne].classList.contains("ingest-step-running")&&J(ne,"error",F.message)}}};return i.addEventListener("submit",ce),t.push(()=>i.removeEventListener("submit",ce)),()=>t.forEach(p=>{try{p()}catch{}})}function cm(e){const t={name:"/",children:{},items:[]};for(const[n,a]of Object.entries(e)){const o=n.split("/").filter(Boolean);if(o.length<=1)t.items.push({path:n,name:o[0]||n,...a});else{let i=t;for(let r=0;r<o.length-1;r++)i.children[o[r]]||(i.children[o[r]]={name:o[r],children:{},items:[]}),i=i.children[o[r]];i.items.push({path:n,name:o[o.length-1],...a})}}return t}function ro(e){let t=e.items.length;for(const n of Object.values(e.children))t+=ro(n);return t}function dm(e){return e?e<1024?e+" B":e<1048576?(e/1024).toFixed(1)+" KB":(e/1048576).toFixed(1)+" MB":""}function so(e,t){let n="";for(const[a,o]of Object.entries(e.children).sort()){const i=ro(o);n+=`<div class="ia-tree-folder">
    ...
    </div>`}for(const a of e.items){const o=dm(a.size),i=a.added?new Date(a.added*1e3).toLocaleDateString():"";n+=`<div class="ia-tree-item" data-path="${a.path}" data-sha256="${a.sha256}" title="${a.path}">
      <span class="ia-tree-item-title">${a.title||a.name}</span>
      <span class="ia-tree-item-meta">${o}${i?" &middot; "+i:""}</span>
    </div>`}return n}function pm(e){const t=[],n=document.getElementById("ia-archive-fab"),a=document.getElementById("ia-archive-overlay"),o=document.getElementById("ia-archive-close"),i=document.getElementById("ia-archive-status"),r=document.getElementById("ia-archive-tree");if(!n||!a)return console.warn("[archive-browser] Required DOM elements not found"),()=>{};async function s(){if(i.textContent="Loading...",r.innerHTML="",!window.nostr||!window.nostr.getPublicKey){i.textContent="Login with NIP-07 extension to view your archive";return}try{const b=await window.nostr.getPublicKey(),w=_e(),h=w.relays.split(",").map(R=>R.trim()).filter(Boolean);i.textContent=`Fetching manifest for ${b.slice(0,8)}...`;const x=await yt(b,w.manifestDtag||"archive",h);if(console.log("[archive] pubkey:",b,"dtag:",w.manifestDtag,"relays:",h),console.log("[archive] manifest found:",x?x.id:"null"),x&&console.log("[archive] manifest content:",x.content),!x){i.textContent="No archive found. Upload files to create your personal archive.",r.innerHTML='<div class="ia-tree-empty">Your archive is empty</div>';return}let T={};try{T=JSON.parse(x.content||"{}").entries||{}}catch{for(const R of x.tags)R[0]==="path"&&R[1]&&R[2]&&(T[R[1]]={sha256:R[2],title:R[1].split("/").pop()})}const C=Object.keys(T).length,$=new Date(x.created_at*1e3).toLocaleString();if(i.textContent=`${C} items, updated ${$}`,C===0){r.innerHTML='<div class="ia-tree-empty">Archive is empty</div>';return}const A=cm(T);r.innerHTML=so(A,0);const P=r.querySelector(".ia-tree-folder");P&&P.classList.add("open"),r.querySelectorAll(".ia-tree-item").forEach(R=>{const B=()=>{const W=R.dataset.sha256,U=`${_e().blossomUrl||"https://blossom.primal.net"}/${W}`;window.open(U,"_blank")};R.addEventListener("click",B),t.push(()=>R.removeEventListener("click",B))})}catch(b){i.textContent=`Error: ${b.message}`}}function c(){a.classList.remove("hidden"),s()}function l(){a.classList.add("hidden")}function d(b){b.target===a&&a.classList.add("hidden")}function u(){a.classList.add("hidden");const b=document.getElementById("ingest-overlay");b&&b.classList.remove("hidden"),document.dispatchEvent(new CustomEvent("ia-switch-tab",{detail:{tab:"sync"}}))}n.addEventListener("click",c),o&&o.addEventListener("click",l),a.addEventListener("click",d);const f=document.getElementById("ia-archive-sync-btn");return f&&f.addEventListener("click",u),t.push(()=>{n.removeEventListener("click",c),o&&o.removeEventListener("click",l),a.removeEventListener("click",d),f&&f.removeEventListener("click",u)}),()=>t.forEach(b=>{try{b()}catch{}})}const um="modulepreload",mm=function(e){return"/bud-lite/"+e},la={},fm=function(t,n,a){let o=Promise.resolve();if(n&&n.length>0){let r=function(l){return Promise.all(l.map(d=>Promise.resolve(d).then(u=>({status:"fulfilled",value:u}),u=>({status:"rejected",reason:u}))))};document.getElementsByTagName("link");const s=document.querySelector("meta[property=csp-nonce]"),c=(s==null?void 0:s.nonce)||(s==null?void 0:s.getAttribute("nonce"));o=r(n.map(l=>{if(l=mm(l),l in la)return;la[l]=!0;const d=l.endsWith(".css"),u=d?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${l}"]${u}`))return;const f=document.createElement("link");if(f.rel=d?"stylesheet":um,d||(f.as="script"),f.crossOrigin="",f.href=l,c&&f.setAttribute("nonce",c),document.head.appendChild(f),d)return new Promise((b,w)=>{f.addEventListener("load",b),f.addEventListener("error",()=>w(new Error(`Unable to preload CSS for ${l}`)))})}))}function i(r){const s=new Event("vite:preloadError",{cancelable:!0});if(s.payload=r,window.dispatchEvent(s),!s.defaultPrevented)throw r}return o.then(r=>{for(const s of r||[])s.status==="rejected"&&i(s.reason);return t().catch(i)})};function Ae(e){return(e||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function hm(e){let t=Ae(e);return t=t.replace(/^### (.+)$/gm,"<h4>$1</h4>"),t=t.replace(/^## (.+)$/gm,"<h3>$1</h3>"),t=t.replace(/^# (.+)$/gm,"<h3>$1</h3>"),t=t.replace(/\*\*\*(.+?)\*\*\*/g,"<strong><em>$1</em></strong>"),t=t.replace(/\*\*(.+?)\*\*/g,"<strong>$1</strong>"),t=t.replace(new RegExp("(?<!\\*)\\*(.+?)\\*(?!\\*)","g"),"<em>$1</em>"),t=t.replace(/`([^`]+)`/g,"<code>$1</code>"),t=t.replace(/^[\-•] (.+)$/gm,"<li>$1</li>"),t=t.replace(/(<li>.*<\/li>)/gs,"<ul>$1</ul>"),t=t.replace(/^\d+\. (.+)$/gm,"<li>$1</li>"),t=t.replace(/\n\n/g,"</p><p>"),t=t.replace(/\n/g,"<br>"),t.startsWith("<")||(t="<p>"+t+"</p>"),t}function gm(e,t){return e.replace(/\[(\d+)\]/g,(n,a)=>`<a class="ia-cite" href="javascript:void(0)" data-msg="${t}" data-idx="${parseInt(a)-1}" onclick="window.__scrollToSource(this)">[${a}]</a>`)}function ca(e,t){return e?"https://primal.net/e/"+e:""}function da(e){return e.endsWith("?")||/^(what|who|how|why|when|where|which|can|does|is|are|do|tell|explain)/i.test(e)}function pa(e,t){if(!e&&!t)return"";let n='<div class="ia-prompt-section">';return n+=`<div class="ia-sources-header" onclick="this.nextElementSibling.classList.toggle('open')">Prompt ▾</div>`,n+='<div class="ia-prompt-body">',e&&(n+=`<div style="padding:0.5em;border-bottom:1px solid #222"><span class="ia-prompt-label">SYSTEM</span><pre>${Ae(e)}</pre></div>`),t&&(n+=`<div style="padding:0.5em"><span class="ia-prompt-label">USER</span><pre>${Ae(t)}</pre></div>`),n+="</div></div>",n}window.__scrollToSource=function(e){const t=e.getAttribute("data-msg"),n=e.getAttribute("data-idx"),a=document.getElementById("sources-body-"+t);a&&(a.style.display="block"),setTimeout(()=>{const o=document.getElementById("src-"+t+"-"+n);o&&(o.scrollIntoView({behavior:"smooth",block:"center"}),o.style.background="#2a3a5f")},50)};const lo="rag-chat-sessions";function Xe(){try{return JSON.parse(localStorage.getItem(lo)||"{}")}catch{return{}}}function Lt(e){localStorage.setItem(lo,JSON.stringify(e))}function ym(e){return Xe()[e]||{id:e,created:Date.now(),messages:[]}}function bm(e){const t=Ct();e.innerHTML=`
  ...
  `;const n=e.querySelector("#rag-chat"),a=e.querySelector("#rag-input"),o=e.querySelector("#rag-ask-btn"),i=e.querySelector("#rag-user-bar"),r=e.querySelector("#rag-history-bar");let s=[],c=new Set,l=null,d=0,u="";ao().then(v=>{u=v}).catch(()=>{}),Ct().llm||B();async function b(){try{s=await oo(),c=new Set(s.map(v=>v.pubkey)),x(),h()}catch(v){console.error("Failed to load pubkeys:",v)}}let w={};async function h(){if(s.length)try{const{countNotesPerPubkey:v}=await fm(async()=>{const{countNotesPerPubkey:L}=await Promise.resolve().then(()=>tm);return{countNotesPerPubkey:L}},void 0);w=await v(s.map(L=>L.pubkey)),x()}catch{}}function x(){i.querySelectorAll(".ia-user-chip").forEach(v=>v.remove());for(const v of s){const L=document.createElement("span");L.className="ia-user-chip "+(c.has(v.pubkey)?"active":"inactive");const U=v.label||v.name||v.pubkey.slice(0,8),N=Ae(U[0].toUpperCase()),H=w[v.pubkey],ee=H!==void 0?`<span class="ia-chip-count">${H}</span>`:"",be=v.picture?`<img src="${Ae(v.picture)}" class="pfp-img"><span class="no-pfp">${N}</span>`:`<span class="no-pfp">${N}</span>`,ie=v.micro?`<div class="ia-chip-tooltip">${Ae(v.micro)}</div>`:"";L.innerHTML=be+`<span>${Ae(U)}</span>${ee}`+ie,L.onclick=()=>{if(c.has(v.pubkey)){if(c.size<=1)return;c.delete(v.pubkey)}else c.add(v.pubkey);x()},i.appendChild(L)}}function T(){return c.size===s.length?[]:Array.from(c)}b();function C(){r.querySelectorAll(".ia-session-tab").forEach(U=>U.remove());const v=Xe(),L=Object.keys(v).sort((U,N)=>v[N].created-v[U].created);for(const U of L){const N=v[U],H=document.createElement("span");H.className="ia-session-tab"+(U===l?" active":"");const ee=N.messages.length?N.messages[0].text.slice(0,25):"Empty";H.innerHTML=Ae(ee)+(U===l?"":` <span class="tab-close" onclick="event.stopPropagation();window.__deleteRagSession('${U}')">×</span>`),H.onclick=()=>R(U),r.insertBefore(H,r.querySelector("#rag-new-session"))}}window.__deleteRagSession=function(v){const L=Xe();delete L[v],Lt(L),v===l&&(l=null,P()),C()};function $(){if(!l)return;const v=[];n.querySelectorAll(".ia-chat-msg").forEach(U=>{const N=U.classList.contains("user");v.push({type:N?"user":"ai",text:N?U.textContent:"",html:N?"":U.innerHTML})});const L=Xe();L[l]||(L[l]={id:l,created:Date.now()}),L[l].messages=v,Lt(L)}function A(v,L,U){const N=document.createElement("div");if(N.className="ia-chat-msg "+v,N.innerHTML=L,v==="ai"){const H=document.createElement("button");H.className="ia-copy-btn",H.textContent="copy",H.onclick=function(){const ee=N.querySelector(".ia-answer-block")||N;navigator.clipboard.writeText(ee.textContent.trim()).then(()=>{H.textContent="copied",setTimeout(()=>{H.textContent="copy"},1500)})},N.appendChild(H)}if(n.appendChild(N),n.scrollTop=n.scrollHeight,U&&l){const H=Xe();H[l]||(H[l]={id:l,created:Date.now(),messages:[]}),H[l].messages.push({type:v,text:v==="user"?N.textContent:"",html:v==="user"?"":N.innerHTML}),Lt(H),C()}}function P(){l&&$(),l="s"+Date.now();const v=Xe();v[l]={id:l,created:Date.now(),messages:[]},Lt(v),n.innerHTML="",d=0,A("ai",'Search or ask questions about indexed Nostr notes. Try: <em>"what is the best hardware wallet?"</em> or <em>"lightning network"</em>',!1),C()}function R(v){l&&$(),l=v;const L=ym(v);n.innerHTML="",d=0;for(const U of L.messages)d++,U.type==="user"?A("user",Ae(U.text),!1):A("ai",U.html,!1);L.messages.length||(n.innerHTML="",A("ai",'Search or ask questions about indexed Nostr notes. Try: <em>"what is the best hardware wallet?"</em> or <em>"lightning network"</em>',!1)),C()}(function(){const v=Xe(),L=Object.keys(v).sort((U,N)=>v[N].created-v[U].created);L.length?R(L[0]):P(),C()})(),e.querySelector("#rag-new-session").onclick=P;function B(){const v=document.getElementById("ia-key-modal");v&&v.remove();const L=_e(),U=!!L.llmApiKey,N=document.createElement("div");N.id="ia-key-modal",N.style.cssText="position:fixed;inset:0;z-index:1000;display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,0.6)",N.innerHTML=`
      ...
      </div>`,document.body.appendChild(N);const H=N.querySelector("#ia-key-input"),ee=N.querySelector("#ia-key-url"),be=N.querySelector("#ia-key-model");U&&(ee.value=L.llmBaseUrl),U&&(be.value=L.llmModel),H.focus(),N.querySelector("#ia-key-cancel").onclick=()=>{N.remove()},N.querySelector("#ia-key-save").onclick=()=>{const ie=_e(),ce=H.value.trim();ce&&ce!=="••••••••"&&(ie.llmApiKey=ce),ee.value.trim()&&(ie.llmBaseUrl=ee.value.trim()),be.value.trim()&&(ie.llmModel=be.value.trim()),fa(ie),N.remove()},N.onclick=ie=>{ie.target===N&&N.remove()}}async function W(){const v=a.value.trim();if(!v)return;if(!Ct().llm&&da(v)){B();return}a.value="",a.style.height="auto",o.disabled=!0;const U="m"+ ++d,N=T();if(A("user",Ae(v)+'<div id="prompt-'+U+'">'+pa(u,"")+"</div>",!0),da(v)){var H=null,ee=document.getElementById("prompt-"+U);try{if(H=await no(v,{pubkeys:N}),ee){var be=ee.querySelector(".ia-prompt-body"),ie=be&&be.classList.contains("open");if(ee.innerHTML=pa(H.system_prompt,H.user_prompt),ie){var ce=ee.querySelector(".ia-prompt-body");ce&&ce.classList.add("open")}}}catch(p){console.error("Preview failed:",p)}A("ai",'<span class="ia-typing">Thinking</span>',!1);try{const p=await to(v,{pubkeys:N});n.lastChild.remove();let m=hm(p.answer);m=gm(m,U);let g="";if(p.sources&&p.sources.length){g='<div class="ia-sources-section">',g+=`<div class="ia-sources-header" onclick="var n=this.nextElementSibling;n.style.display=n.style.display==='none'?'block':'none'">Sources (${p.sources.length}) ▾</div>`,g+=`<div class="ia-sources-body" id="sources-body-${U}" style="display:none">`;for(let k=0;k<p.sources.length;k++){const E=p.sources[k],X=E.date?new Date(E.date*1e3).toLocaleDateString():"",re=E.event_id?`<a class="ia-primal-link" href="${ca(E.event_id,E.kind)}" target="_blank">open ↗</a>`:"";g+=`<div class="ia-src-note" id="src-${U}-${k}"><div class="ia-src-header"><span class="ia-src-num">[${k+1}]</span> <span style="color:#93c5fd">${Ae(E.author)}</span> <span class="meta">${X} ${(E.score*100).toFixed(0)}%</span> `+re+`</div><div class="ia-src-text">${Ae(E.content)}</div></div>`}g+="</div></div>"}A("ai",`<div class="ia-answer-block">${m}</div>${g}`,!0)}catch(p){n.lastChild.remove(),A("ai",`<div class="ia-error-msg"><strong>Request failed</strong><br>${Ae(p.message)}</div>`,!0)}}else{A("ai",'<span class="ia-typing">Searching</span>',!1);try{const p=await eo(v,{pubkeys:N});if(n.lastChild.remove(),p.error)A("ai",`<div class="ia-error-msg">${Ae(p.error)}</div>`,!0);else if(!p.count)A("ai","No results found.",!0);else{let m=`<strong>${p.count} results</strong>`;for(const g of p.results){const k=g.created_at?new Date(g.created_at*1e3).toLocaleDateString():"",E=g.event_id?`<a class="ia-primal-link" href="${ca(g.event_id,g.kind)}" target="_blank">open ↗</a>`:"";m+=`<div class="ia-result-card"><span class="score">${(g.score*100).toFixed(1)}%</span> <span class="meta">${Ae(g.author_label||"?")} · ${k}</span> `+E+`<br>${Ae(g.content.slice(0,300))}${g.content.length>300?"...":""}</div>`}A("ai",m,!0)}}catch(p){n.lastChild.remove(),A("ai",`<div class="ia-error-msg"><strong>Search failed</strong><br>${Ae(p.message)}</div>`,!0)}}o.disabled=!1,a.focus()}return a.addEventListener("keydown",v=>{v.key==="Enter"&&!v.shiftKey&&(v.preventDefault(),W())}),a.addEventListener("input",()=>{a.style.height="auto",a.style.height=Math.min(a.scrollHeight,128)+"px"}),o.addEventListener("click",W),function(){}}function Ht(e,t){const n=Fe(e.mime),a=Je[n]||"📄",o=bt(e.size),i=e.added?new Date(e.added*1e3).toLocaleString(void 0,{year:"numeric",month:"short",day:"numeric"}):"",r=(e.mime||"").split("/").pop().split(";")[0].replace("application/","").replace("text/",""),s=(e.mime||"").toLowerCase(),c=`${t}/${e.sha256}`,l=s.startsWith("image/")?`<img src="${c}" alt="${te(e.title)}" class="h-full w-full object-cover" loading="lazy">`:`<span class="text-5xl">${a}</span>`,d=document.createElement("a");return d.href="#/details/"+e.sha256,d.className="relative flex flex-col rounded-sm border border-[var(--color-ia-border)] bg-white text-inherit hover:shadow hover:no-underline",d.innerHTML=`
    ...
    <div class="p-2">
    ...
    </div>`,d.dataset.sha256=e.sha256,d.dataset.manifestId=e.manifestId||"",d.dataset.manifestPubkey=e.manifestPubkey||"",d.dataset.manifestItem="1",d.dataset.title=(e.title||"").toLowerCase(),d}function Vt(e,t={}){const n=document.createElement("input");n.type="text",n.placeholder=t.placeholder||"Search files...",n.className="rounded-sm border border-[var(--color-ia-border)] bg-white px-3 py-1 text-sm text-[var(--color-ia-ink)] placeholder:text-zinc-400 outline-none focus:border-[var(--color-ia-link)]",n.style.width=t.width||"200px";function a(){const i=n.value.toLowerCase();e.querySelectorAll("a[data-manifest-item]").forEach(s=>{const c=s.dataset.title||"";s.style.display=c.includes(i)?"":"none"})}n.addEventListener("input",a);function o(){n.removeEventListener("input",a),n.remove()}return{el:n,cleanup:o}}let ge=null,$t=null;function wm(){if(ge)return;ge=document.createElement("div"),ge.id="ia-raw-modal",ge.className="hidden",ge.style.cssText=["position: fixed","inset: 0","z-index: 99998","background: rgba(0,0,0,0.7)","display: flex","align-items: center","justify-content: center","backdrop-filter: blur(4px)"].join("; ");const e=document.createElement("div");e.id="ia-raw-content",e.style.cssText=["width: 90%","max-width: 700px","max-height: 80vh","overflow: auto","background: #1a1a2e","border: 1px solid #333","border-radius: 8px","padding: 1.2em","color: #d8dadf","font-family: monospace","font-size: 12px","white-space: pre-wrap","word-break: break-all","position: relative"].join("; ");const t=document.createElement("button");t.id="ia-raw-close",t.innerHTML="&times;",t.style.cssText=["position: absolute","top: 8px","right: 12px","background: none","border: none","color: #888","font-size: 1.3rem","cursor: pointer"].join("; "),t.addEventListener("click",()=>{ge.classList.add("hidden"),ge.style.display="none"}),$t=document.createElement("pre"),$t.id="ia-raw-json",e.appendChild(t),e.appendChild($t),ge.appendChild(e),document.body.appendChild(ge),ge.addEventListener("click",n=>{n.target===ge&&(ge.classList.add("hidden"),ge.style.display="none")})}function vm(e){wm(),$t.textContent=JSON.stringify(e,null,2),ge.classList.remove("hidden"),ge.style.display="flex";function t(a){a.key==="Escape"&&(ge.classList.add("hidden"),ge.style.display="none",document.removeEventListener("keydown",t))}document.addEventListener("keydown",t);function n(){document.removeEventListener("keydown",t),ge&&(ge.classList.add("hidden"),ge.style.display="none")}return{cleanup:n}}const xm=["wss://nos.lol","wss://relay.damus.io"];let He=null;const en=new Map;async function tn(e){if(en.has(e))return en.get(e);for(const t of xm)try{const n=new WebSocket(t),a=await new Promise((o,i)=>{const r=setTimeout(()=>{n.close(),i("timeout")},5e3);n.onopen=()=>n.send(JSON.stringify(["REQ","kebab",{ids:[e],limit:1}])),n.onmessage=s=>{const c=JSON.parse(s.data);c[0]==="EVENT"&&c[2]&&(clearTimeout(r),n.close(),o(c[2]))},n.onerror=()=>{clearTimeout(r),i("ws error")}});return en.set(e,a),a}catch{continue}return null}function vt(){He&&(He.btn.classList.remove("ia-kebab-open"),He.menu.remove(),He=null)}async function _m(e,t){vt();const n=document.createElement("div");n.className="ia-dropdown";const a=[{label:"Copy Event ID",action:async()=>{await navigator.clipboard.writeText(t),Pe("Event ID copied")}},{label:"Copy Raw JSON",action:async()=>{const o=await tn(t);o?(await navigator.clipboard.writeText(JSON.stringify(o,null,2)),Pe("JSON copied")):Pe("Failed to fetch event")}},{label:"View Raw Data",action:async()=>{const o=await tn(t);o?vm(o):Pe("Failed to fetch event")}},{label:"Edit",action:async()=>{window.location.hash=`/details/${t}`}},{label:"Delete",danger:!0,action:async()=>{if(confirm("Delete this item? This removes it from your archive manifest.")){if(!window.nostr){Pe("NIP-07 signer required");return}try{const o=await window.nostr.getPublicKey(),i=_e(),r=i.relays.split(",").map(h=>h.trim()).filter(Boolean),s=await yt(o,i.manifestDtag||"archive",r);if(!s){Pe("No manifest found");return}const c=ot(s.content);let l=!1;for(const[h,x]of Object.entries(c))if(x.index_event_id===t||x.bridge_event_id===t){delete c[h],l=!0;break}if(!l){Pe("Item not found in manifest");return}const d=rn(c),u=await an(d),f=[...d,["d",i.manifestDtag||"archive"],["x",u,"aggregate"],["title","Personal Archive"],["server",i.blossomUrl]],b={kind:35128,content:on(c),created_at:Math.floor(Date.now()/1e3),tags:f},w=await window.nostr.signEvent(b);await at(w,r),Pe("Deleted from manifest"),setTimeout(()=>location.reload(),1e3)}catch(o){Pe("Delete failed: "+o.message)}}}}];for(const o of a){const i=document.createElement("button");i.className="ia-dropdown-item"+(o.danger?" ia-danger":""),i.textContent=o.label,i.addEventListener("click",async r=>{r.stopPropagation(),r.preventDefault(),vt(),await o.action()}),n.appendChild(i)}e.classList.add("ia-kebab-open"),e.parentElement.style.position="relative",e.parentElement.appendChild(n),He={btn:e,menu:n},tn(t)}function nt(){document.querySelectorAll('a[href*="/details/"]').forEach(t=>{var o;if(t.querySelector(".ia-kebab"))return;const n=t.dataset.manifestId||((o=t.getAttribute("href"))==null?void 0:o.split("/details/")[1])||"";if(!n)return;const a=document.createElement("button");a.className="ia-kebab",a.textContent="⋮",a.title="Actions",a.addEventListener("click",i=>{i.preventDefault(),i.stopPropagation(),He&&He.btn===a?vt():_m(a,n)}),t.style.position="relative",t.appendChild(a)})}function zt(e,t){const n=[];function a(l){He&&!He.menu.contains(l.target)&&l.target!==He.btn&&vt()}document.addEventListener("click",a),n.push(()=>document.removeEventListener("click",a));const o=[];o.push(setTimeout(nt,2e3)),o.push(setTimeout(nt,5e3)),o.push(setTimeout(nt,1e4));const i=setInterval(nt,8e3);function r(){setTimeout(nt,500)}window.addEventListener("hashchange",r),n.push(()=>window.removeEventListener("hashchange",r));let s=null;e&&(s=new MutationObserver(()=>{nt()}),s.observe(e,{childList:!0,subtree:!0}));function c(){n.forEach(l=>{try{l()}catch{}}),o.forEach(l=>clearTimeout(l)),clearInterval(i),s&&s.disconnect(),vt()}return c}const co="ia-view-mode",po="ia-list-sort";function cn(){return localStorage.getItem(co)||"grid"}function dn(){return localStorage.getItem(po)||"date-desc"}function uo(e){localStorage.setItem(co,e),document.documentElement.setAttribute("data-view",e),document.querySelectorAll(".ia-view-toggle button").forEach(t=>t.classList.toggle("active",t.dataset.mode===e)),e==="list"?(Sn(),Tn()):(Im(),document.querySelectorAll(".ia-items-grid a").forEach(t=>t.style.order=""))}function km(){const e=document.createElement("span");e.className="ia-view-toggle";const t=cn();return e.innerHTML=`<button data-mode="grid" class="${t==="grid"?"active":""}" title="Grid view">&#9638;</button><button data-mode="list" class="${t==="list"?"active":""}" title="List view">&#9776;</button>`,e.querySelectorAll("button").forEach(n=>n.addEventListener("click",()=>uo(n.dataset.mode))),e}function Am(e){const n=e.textContent.match(/([\d.]+)\s*(B|KB|MB|GB)/i);if(!n)return 0;const a=parseFloat(n[1]),o=n[2].toUpperCase();return a*(o==="GB"?1e9:o==="MB"?1e6:o==="KB"?1e3:1)}function Tm(e){var t,n;return((n=(t=e.querySelector("[class*='font-mono']"))==null?void 0:t.textContent)==null?void 0:n.toLowerCase())||""}function Sm(e){const t=e.textContent,n=t.match(/(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\s+\d+,?\s+\d+,?\s+\d+:\d+\s*(AM|PM)/i);if(n){const o=new Date(n[0]);if(!isNaN(o))return o.getTime()}const a=t.match(/(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\s+\d+,?\s+\d+/i);return a?new Date(a[0]).getTime():0}function Tn(){const e=dn(),[t,n]=e.split("-");document.querySelectorAll(".ia-items-grid").forEach(a=>{const i=Array.from(a.children).map(r=>{var c,l;let s;return t==="name"?s=((l=(c=r.querySelector("h3"))==null?void 0:c.textContent)==null?void 0:l.toLowerCase())||"":t==="size"?s=Am(r):t==="type"?s=Tm(r):s=Sm(r),{el:r,v:s}});i.sort((r,s)=>n==="asc"?r.v>s.v?1:-1:r.v<s.v?1:-1),i.forEach((r,s)=>r.el.style.order=s)})}function Sn(){document.querySelectorAll(".ia-list-header").forEach(e=>{const t=e.nextElementSibling;(!t||!t.classList.contains("ia-items-grid"))&&e.remove()}),document.querySelectorAll(".ia-items-grid").forEach(e=>{var a;if((a=e.previousElementSibling)!=null&&a.classList.contains("ia-list-header"))return;const t=dn(),n=document.createElement("div");n.className="ia-list-header",n.innerHTML=[{col:"name",label:"Name"},{col:"size",label:"Size"},{col:"type",label:"Type"},{col:"date",label:"Date"}].map(o=>{const i=t.startsWith(o.col)?t.endsWith("asc")?"sort-asc":"sort-desc":"";return`<span data-col="${o.col}" class="${i}">${o.label}</span>`}).join(""),n.querySelectorAll("span").forEach(o=>o.addEventListener("click",()=>{const i=o.dataset.col,s=dn()===i+"-asc"?i+"-desc":i+"-asc";localStorage.setItem(po,s),Sn(),Tn()})),e.parentElement.insertBefore(n,e)})}function Im(){document.querySelectorAll(".ia-list-header").forEach(e=>e.remove())}function Em(){document.querySelectorAll("h2").forEach(e=>{if(e.querySelector(".ia-view-toggle"))return;const t=e.closest("section")||e.closest("div");t&&t.querySelector(".ia-items-grid")&&e.appendChild(km())})}function In(e,t){const n=[];uo(cn());function a(){Em(),cn()==="list"&&(Sn(),Tn())}const o=[];o.push(setTimeout(a,1500)),o.push(setTimeout(a,4e3));const i=setInterval(a,5e3);function r(){setTimeout(a,300)}window.addEventListener("hashchange",r),n.push(()=>window.removeEventListener("hashchange",r));function s(){n.forEach(c=>{try{c()}catch{}}),o.forEach(c=>clearTimeout(c)),clearInterval(i)}return s}function Lm(e,t){const n=document.querySelector("main");n.innerHTML="";const a=[],o=document.createElement("section");o.className="py-6";const i=document.createElement("div");i.className="mx-auto max-w-[1280px] px-4 mb-4 flex items-center justify-between";const r=document.createElement("h2");r.className="text-sm font-semibold uppercase tracking-wider text-zinc-500",r.textContent="Latest additions",i.appendChild(r),a.push(In());const s=document.createElement("div");s.className="ia-items-grid",s.id="manifest-feed-grid";const c=document.createElement("div");c.className="mx-auto max-w-[1280px] px-4",c.appendChild(i),c.appendChild(s),o.appendChild(c);const{el:l,cleanup:d}=Vt(s);i.appendChild(l),a.push(d),a.push(zt(s));function u(f){const{user:b,blossomBase:w}=t.getState();for(;n.firstChild&&n.firstChild!==o;)n.removeChild(n.firstChild);if(!(b!=null&&b.pubkey)){o.style.display="none";const{exploreArchives:P}=t.getState(),R=document.createElement("section");R.className="bg-gradient-to-b from-[var(--color-ia-nav-hover)] to-[var(--color-ia-nav)] py-8 text-white",R.innerHTML=`
      ...
      <div class="mx-auto max-w-[1280px] px-4 flex flex-wrap gap-3">
        ...
        </div>`,n.insertBefore(P,o)}s.innerHTML="";const A=new Set;for(const P of f.slice(0,24))A.has(P.sha256)||(A.add(P.sha256),s.appendChild(Ht(P,w)))}return n.appendChild(o),u(t.getState().items),a.push(t.subscribe("items",f=>{u(f)})),a.push(t.subscribe("user",()=>{u(t.getState().items)})),a.push(t.subscribe("exploreArchives",()=>{var f;(f=t.getState().user)!=null&&f.pubkey||u(t.getState().items)})),function(){a.forEach(b=>{try{b()}catch{}})}}function Kt(e){const t=document.createElement("section");t.className="border-b border-[var(--color-ia-border)] bg-[var(--color-ia-bg-soft)] py-4";const n=document.createElement("nav");n.className="text-[11px] text-zinc-500 mx-auto max-w-[1280px] px-4";const a=e.map((o,i)=>i===e.length-1?`<span class="text-zinc-700">${te(o.label)}</span>`:`<a class="hover:underline" href="${te(o.href||"#/")}">${te(o.label)}</a>`);return n.innerHTML=a.join(" › "),t.appendChild(n),t}function Pm(e,t){const[n]=e,a=document.querySelector("main");a.innerHTML="";const{items:o,blossomBase:i}=t.getState(),r=[],s=o.filter(A=>Fe(A.mime)===n),c=n.charAt(0).toUpperCase()+n.slice(1),l=Je[n]||"📄";a.appendChild(Kt([{label:"Home",href:"#/"},{label:c}]));const d=document.createElement("section");d.className="py-6";const u=document.createElement("div");u.className="mx-auto max-w-[1280px] px-4";const f=document.createElement("div");f.className="mb-4 flex items-center justify-between";const b=document.createElement("h2");b.className="text-sm font-semibold uppercase tracking-wider text-zinc-500",b.textContent=`${l} ${c} — ${s.length} files`,f.appendChild(b),r.push(In());const w={};for(const A of s){const P=A.path.split("/").filter(Boolean);if(P.length>=3){const R=P.slice(1,-1).join("/");w[R]=(w[R]||0)+1}}const h=Object.keys(w).sort();let x=null;h.length>0&&(x=document.createElement("div"),x.className="mb-4 flex flex-wrap gap-2",x.innerHTML=h.map(A=>`<span class="inline-flex items-center gap-1 rounded-full border border-[var(--color-ia-border)] bg-white px-3 py-1 text-xs text-zinc-600 cursor-pointer hover:bg-zinc-50" data-folder="${te(A)}">📁 ${te(A)} <span class="text-zinc-400">(${w[A]})</span></span>`).join(""));const T=document.createElement("div");T.className="ia-items-grid",T.id="manifest-collection-grid",u.appendChild(f),x&&u.appendChild(x),u.appendChild(T),d.appendChild(u),a.appendChild(d);const{el:C,cleanup:$}=Vt(T);f.appendChild(C),r.push($);for(const A of s)T.appendChild(Ht(A,i));return r.push(zt(T)),x&&x.querySelectorAll("[data-folder]").forEach(A=>{A.addEventListener("click",()=>{const P=A.dataset.folder;T.querySelectorAll("a[data-manifest-item]").forEach(R=>{var L;const B=s.find(U=>U.sha256===R.dataset.sha256),W=((L=B==null?void 0:B.path)==null?void 0:L.split("/").filter(Boolean))||[],v=W.length>=3?W.slice(1,-1).join("/"):"";R.style.display=v===P?"":"none"}),x.querySelectorAll("[data-folder]").forEach(R=>R.classList.remove("bg-[var(--color-ia-link)]","text-white")),A.classList.add("bg-[var(--color-ia-link)]","text-white")})}),function(){r.forEach(P=>{try{P()}catch{}})}}function Cm(e,t){const n=decodeURIComponent(e[0]),a=document.querySelector("main");a.innerHTML="";const{items:o,blossomBase:i}=t.getState(),r=[],s=o.filter(h=>{const x=h.path.split("/").filter(Boolean);return x.length>=2&&x[0]===n});a.appendChild(Kt([{label:"Home",href:"#/"},{label:n}]));const c=document.createElement("section");c.className="py-6";const l=document.createElement("div");l.className="mx-auto max-w-[1280px] px-4";const d=document.createElement("div");d.className="mb-4 flex items-center justify-between";const u=document.createElement("h2");u.className="text-sm font-semibold uppercase tracking-wider text-zinc-500",u.textContent=`📁 ${n} — ${s.length} files`,d.appendChild(u),r.push(In());const f=document.createElement("div");f.className="ia-items-grid",f.id="manifest-folder-grid",l.appendChild(d),l.appendChild(f),c.appendChild(l),a.appendChild(c);const{el:b,cleanup:w}=Vt(f);d.appendChild(b),r.push(w);for(const h of s)f.appendChild(Ht(h,i));return r.push(zt(f)),function(){r.forEach(x=>{try{x()}catch{}})}}function $m(e,t){var T,C;const[n]=e,a=document.querySelector("main");a.innerHTML="";const{items:o}=t.getState(),i=o.find($=>$.sha256===n);if(!i){a.innerHTML='<div class="mx-auto max-w-[1280px] px-4 py-12 text-center text-zinc-500">Item not found.</div>';return}const s=`${_t()}/${n}`,c=i.added?new Date(i.added*1e3).toLocaleString():"Unknown",l=(i.mime||"").toLowerCase(),d=Fe(i.mime);a.appendChild(Kt([{label:"Home",href:"#/"},{label:d.charAt(0).toUpperCase()+d.slice(1),href:`#/collection/${d}`},{label:i.title}]));let u="",f="";const b="flex gap-2 p-2 bg-zinc-50 border-t border-[var(--color-ia-border)]",w="rounded-sm bg-[var(--color-ia-nav)] px-3 py-1.5 text-sm text-white hover:bg-[var(--color-ia-nav-hover)] no-underline",h="rounded-sm border border-zinc-300 px-3 py-1.5 text-sm text-zinc-600 hover:bg-zinc-100 no-underline";l.startsWith("image/")?u=`<img src="${s}" alt="${te(i.title)}" style="display:block;max-width:100%;max-height:80vh;object-fit:contain;margin:auto">`:l.startsWith("audio/")?u=`<audio controls src="${s}" style="width:100%;display:block"></audio>`:l.startsWith("video/")?u=`<video controls src="${s}" style="display:block;max-width:100%;max-height:80vh;margin:auto"></video>`:l==="text/html"?u='<iframe sandbox="allow-same-origin" srcdoc="" id="ia-html-preview" style="width:100%;min-height:400px;border:none"></iframe>':l.startsWith("text/")||l.includes("json")||l.includes("yaml")||l.includes("javascript")||l.includes("xml")?u='<pre id="ia-text-preview" style="max-height:70vh;overflow:auto;padding:1em;font-size:12px;background:#f9fafb;white-space:pre-wrap;word-break:break-word;margin:0"></pre>':l.includes("pdf")?u=`<iframe src="${s}" style="width:100%;min-height:600px;border:none"></iframe>`:u=`<div class="grid place-items-center py-12 text-zinc-400"><span class="text-6xl">${Je[d]||"📄"}</span><p class="mt-3">No preview available</p></div>`,f=`<div class="${b}"><a href="${s}" download="${te(i.title)}" class="${w}">Download</a><a href="${s}" target="_blank" rel="noopener" class="${h}">Open on Blossom</a></div>`;const x=document.createElement("div");if(x.innerHTML=`
    ...
    </div>`,function(){}}async function Um(e,t){const[n]=e,a=document.querySelector("main");a.innerHTML="";const o=[],i=it(),r=_t(),s=await Do(n,i),c=(s==null?void 0:s.name)||n.slice(0,16)+"...",l=(s==null?void 0:s.picture)||null;a.appendChild(Kt([{label:"Home",href:"#/"},{label:c}]));const d=document.createElement("section");d.className="border-b border-[var(--color-ia-border)] bg-white py-4";const u=l?`<img src="${te(l)}" class="h-12 w-12 rounded-full object-cover flex-shrink-0" onerror="this.outerHTML='<div class=\\'ia-archive-avatar-fallback\\'>&#9786;</div>'">`:'<div class="ia-archive-avatar-fallback">&#9786;</div>',f=s!=null&&s.about?`<p class="mt-1 text-sm text-zinc-600 line-clamp-2">${te(s.about)}</p>`:"",b=s!=null&&s.nip05?`<p class="text-xs text-[var(--color-ia-link)]">${te(s.nip05)}</p>`:"";d.innerHTML=`
    <div class="mx-auto max-w-[1280px] px-4 flex items-start gap-3">
    ...
    </div>`,a.appendChild(d);const w=document.createElement("div");a.appendChild(w);const h=document.createElement("div");a.appendChild(h);const x=document.createElement("section");x.className="py-6";const T=document.createElement("div");T.className="mx-auto max-w-[1280px] px-4";const C=document.createElement("div");C.className="mb-4 flex items-center justify-between";const $=document.createElement("h2");$.className="text-sm font-semibold uppercase tracking-wider text-zinc-500",$.textContent="Loading...",C.appendChild($);const A=document.createElement("div");A.className="ia-items-grid",A.id="archive-grid";const{el:P,cleanup:R}=Vt(A);C.appendChild(P),o.push(R),o.push(zt(A)),T.appendChild(C),T.appendChild(A),x.appendChild(T),a.appendChild(x);const B=[];await xt(i,{kinds:[35128],authors:[n],limit:50},p=>{const m=un(p);B.push(...m)}),B.sort((p,m)=>(m.added||0)-(p.added||0));let W=null,v=null;function L(){return B.filter(p=>p.path.split("/").filter(Boolean).length<=1)}function U(){let p=B;return W&&(p=p.filter(m=>Fe(m.mime)===W)),v&&(p=p.filter(m=>{const g=m.path.split("/").filter(Boolean);return g.length>=2&&g[0]===v})),p}function N(p){const m={};for(const g of p){const k=Fe(g.mime);m[k]=(m[k]||0)+1}return m}function H(p){const m={};for(const g of p){const k=g.path.split("/").filter(Boolean);if(k.length>=2){const E=k[0];m[E]||(m[E]={count:0,kind:Fe(g.mime)}),m[E].count++}}return m}function ee(){return W!==null||v!==null}function be(){const p=N(B),m=ee()?U():L();$.textContent=`${m.length} file${m.length!==1?"s":""}`,w.innerHTML="";const g=document.createElement("section");g.className="border-b border-[var(--color-ia-border)] bg-white py-3",g.innerHTML=`
      <div class="mx-auto max-w-[1280px] px-4 flex flex-wrap gap-2">
        ...
        ${Object.entries(p).map(([k,E])=>{const X=Je[k]||"📄",re=k.charAt(0).toUpperCase()+k.slice(1);return`<button class="ia-filter-chip ${W===k?"active":""}" data-kind="${k}">${X} ${re} ${E}</button>`}).join("")}
      </div>`,g.querySelectorAll('[data-filter="all"]').forEach(k=>{k.addEventListener("click",()=>{W=null,v=null,be(),ie(),ce()})}),g.querySelectorAll("[data-kind]").forEach(k=>{k.addEventListener("click",()=>{const E=k.dataset.kind;W=W===E?null:E,v=null,be(),ie(),ce()})}),w.appendChild(g)}function ie(){h.innerHTML="";const p=W?B.filter(E=>Fe(E.mime)===W):B,m=H(p),g=Object.keys(m).sort();if(g.length===0)return;const k=document.createElement("section");k.className="border-b border-[var(--color-ia-border)] bg-white py-3",k.innerHTML=`
      <div class="mx-auto max-w-[1280px] px-4">
        <div class="flex flex-wrap gap-2">
          ${g.map(E=>{const X=m[E],re=Je[X.kind]||"📁";return`<button class="ia-filter-chip ${v===E?"active":""}" data-folder="${te(E)}">${re} ${te(E)} ${X.count}</button>`}).join("")}
        </div>
      </div>`,k.querySelectorAll("[data-folder]").forEach(E=>{E.addEventListener("click",()=>{const X=E.dataset.folder;v=v===X?null:X,W=null,be(),ie(),ce()})}),h.appendChild(k)}function ce(){const p=ee()?U():L();if(A.innerHTML="",p.length===0&&!ee()){A.innerHTML='<div class="col-span-full text-center py-8 text-zinc-400 text-sm">No files at root level. Browse collections or folders above.</div>';return}if(p.length===0){A.innerHTML='<div class="col-span-full text-center py-8 text-zinc-400 text-sm">No files match this filter.</div>';return}const m=new Set;for(const g of p)m.has(g.sha256)||(m.add(g.sha256),A.appendChild(Ht(g,r)))}return B.length===0?($.textContent="0 files",A.innerHTML='<div class="col-span-full text-center py-12 text-zinc-400 text-sm">No files found in this archive.</div>'):(be(),ie(),ce()),function(){o.forEach(m=>{try{m()}catch{}})}}function Mm(e,t){const n=document.querySelector("main");n.innerHTML="";const a=[],o=document.createElement("section");o.className="bg-gradient-to-b from-[var(--color-ia-nav-hover)] to-[var(--color-ia-nav)] py-8 text-white",o.innerHTML=`
    <div class="mx-auto max-w-[1280px] px-4">
    ...
    </div>`,n.appendChild(o);const i=document.createElement("section");i.className="py-6";const r=document.createElement("div");r.className="mx-auto max-w-[1280px] px-4";const s=document.createElement("div");s.className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3",s.id="explore-grid";const c=document.createElement("button");c.className="mt-6 block mx-auto px-6 py-2 rounded border border-[var(--color-ia-border)] bg-white text-sm text-zinc-600 hover:bg-zinc-50 cursor-pointer",c.textContent="Load More",c.style.display="none";let l=!1;c.addEventListener("click",async()=>{if(!l){l=!0,c.textContent="Loading...";try{window.__loadExploreMore&&await window.__loadExploreMore()}catch(u){console.error("[explore] load more failed:",u)}l=!1,c.textContent="Load More"}}),r.appendChild(s),r.appendChild(c),i.appendChild(r),n.appendChild(i);function d(){const{exploreArchives:u,exploreHasMore:f}=t.getState();if(s.innerHTML="",u.length===0){s.innerHTML=`
        <div class="col-span-full text-center py-12">
          <div class="text-zinc-400 text-sm">Loading archives from relays...</div>
        </div>`,c.style.display="none";return}for(const b of u){const w=b.picture?`<img src="${b.picture}" class="h-10 w-10 rounded-full object-cover" onerror="this.outerHTML='<div class=\\'ia-archive-avatar-fallback\\'>&#9786;</div>'">`:'<div class="ia-archive-avatar-fallback">&#9786;</div>',h=b.recentFiles.length>0?`<ul class="mt-2 space-y-0.5">${b.recentFiles.map(T=>`<li class="text-xs text-zinc-400 truncate">&#8226; ${te(T)}</li>`).join("")}</ul>`:'<div class="text-xs text-zinc-400 mt-1">No files yet</div>',x=document.createElement("a");x.href=`#/archive/${b.pubkey}`,x.className="block rounded-lg border border-[var(--color-ia-border)] bg-white p-4 hover:shadow-md transition-shadow no-underline",x.innerHTML=`
        <div class="flex items-center gap-3">
          ${w}
          <div class="min-w-0 flex-1">
            <div class="font-medium text-sm text-zinc-800 truncate">${te(b.name)}</div>
            <div class="text-xs text-zinc-500">${b.fileCount} files</div>
          </div>
        </div>
        ${h}`,s.appendChild(x)}c.style.display=f?"block":"none"}return d(),a.push(t.subscribe("exploreArchives",d)),function(){a.forEach(f=>{try{f()}catch{}})}}function Nm(e,t){const n=document.getElementById("ia-main");return n.innerHTML="",bm(n),function(){n.innerHTML=""}}const En=300*1e3,ve=$o({user:null,items:[],exploreArchives:[],exploreCacheTime:0,exploreHasMore:!0,followedPubkeys:[],blossomBase:"",cacheTime:0,settings:_e(),viewMode:localStorage.getItem("ia-view-mode")||"grid",listSort:localStorage.getItem("ia-list-sort")||"date-desc",currentRoute:null});document.documentElement.setAttribute("data-view",ve.getState().viewMode);const Dm=document.getElementById("app");Dm.innerHTML=`
  ...
  <button id="ia-settings-fab" class="ia-fab" title="Settings">⚙</button>
    ...
    </div>`;Bo(document.getElementById("ia-login-widget"),ve);nm();lm();pm();async function Ln(){const{user:e,cacheTime:t,items:n}=ve.getState();if(!(e!=null&&e.pubkey)){ve.setState({items:[],cacheTime:0});return}const a=it(),o=_t();if(n.length>0&&t&&Date.now()-t<En){console.log("[feed] using cache",n.length,"items");return}console.log("[feed] streaming from",a,"for",e.pubkey.slice(0,12)+"...");const i=[];await xt(a,{kinds:[35128],authors:[e.pubkey],limit:50},r=>{const s=ot(r.content);for(const[c,l]of Object.entries(s))i.push({title:l.title||c.split("/").pop(),sha256:l.sha256,mime:l.mime,size:l.size,added:l.added||r.created_at,path:c,manifestId:r.id,manifestPubkey:r.pubkey,topics:l.topics||[],source:l.source||null,bridgeEventId:l.bridge_event_id||null,indexEventId:l.index_event_id||null})}),i.sort((r,s)=>(s.added||0)-(r.added||0)),console.log("[feed]",i.length,"items"),ve.setState({items:i,blossomBase:o,cacheTime:Date.now()})}const Pn=20;function mo(e){const{user:t,followedPubkeys:n}=ve.getState(),a={kinds:[35128],limit:Pn};return t!=null&&t.pubkey&&n.length>0&&(a.authors=n),e&&(a.until=e),a}async function fo(e){const t=it();for(const o of e.values())o.items.sort((i,r)=>(r.added||0)-(i.added||0));const n=[...e.keys()],a=await jo(n,t);return n.map(o=>{const{items:i,manifestId:r}=e.get(o),s=a.get(o)||{};return{pubkey:o,name:s.name||o.slice(0,12)+"...",picture:s.picture||null,about:s.about||null,fileCount:i.length,recentFiles:i.slice(0,5).map(c=>c.title),manifestId:r}})}let gt=null;async function Mt(){const{exploreCacheTime:e}=ve.getState();if(e&&Date.now()-e<En){console.log("[explore] using cache");return}const t=it(),n=_t(),a=mo();console.log("[explore] fetching manifests...",a);const o=new Map;let i=1/0;await xt(t,a,s=>{const c=s.pubkey;o.has(c)||o.set(c,{items:[],manifestId:s.id}),o.get(c).items.push(...un(s)),s.created_at<i&&(i=s.created_at)}),gt=i<1/0?i:null;const r=await fo(o);r.sort((s,c)=>c.fileCount-s.fileCount),console.log("[explore]",r.length,"archives"),ve.setState({exploreArchives:r,blossomBase:n,exploreCacheTime:Date.now(),exploreHasMore:r.length>=Pn})}async function jm(){if(!gt)return;const e=it(),t=_t(),n=mo(gt);console.log("[explore] loading more...",n);const a=new Map;let o=1/0;const i=ve.getState().exploreArchives,r=new Set(i.map(l=>l.pubkey));await xt(e,n,l=>{const d=l.pubkey;a.has(d)||a.set(d,{items:[],manifestId:l.id}),a.get(d).items.push(...un(l)),l.created_at<o&&(o=l.created_at)});for(const l of r)a.delete(l);gt=o<1/0?o:gt;const s=await fo(a);if(s.sort((l,d)=>d.fileCount-l.fileCount),s.length===0){ve.setState({exploreHasMore:!1});return}const c=[...i,...s];console.log("[explore] loaded",s.length,"more, total",c.length),ve.setState({exploreArchives:c,blossomBase:t,exploreHasMore:s.length>=Pn})}window.__loadExploreMore=jm;window.__exploreHasMore=()=>ve.getState().exploreHasMore;const Om=Ro([{pattern:/^\/$/,handler:Lm},{pattern:/^\/chat$/,handler:Nm},{pattern:/^\/explore$/,handler:Mm},{pattern:/^\/archive\/([0-9a-f]{64})$/,handler:Um},{pattern:/^\/collection\/(.+)$/,handler:Pm},{pattern:/^\/folder\/(.+)$/,handler:Cm},{pattern:/^\/details\/([0-9a-f]{64})$/,handler:$m},{pattern:/^\/about$/,handler:Rm}],ve);Om.onNavigate=e=>{const t=document.querySelector("footer");t&&(t.style.display=e==="/chat"?"none":"")};{const e=location.hash.replace(/^#/,"")||"/",t=document.querySelector("footer");t&&(t.style.display=e==="/chat"?"none":"")}async function Bm(e){if(e){Ln().catch(a=>console.error("[feed] load failed:",a));const t=it(),n=await Oo(e.pubkey,t);ve.setState({followedPubkeys:n}),console.log("[contacts]",n.length,"followed pubkeys"),ve.setState({exploreCacheTime:0}),Mt().catch(a=>console.error("[explore] load failed:",a))}else ve.setState({items:[],cacheTime:0,followedPubkeys:[],exploreCacheTime:0}),Mt().catch(t=>console.error("[explore] load failed:",t))}ve.subscribe("user",Bm);Xa();ve.getState().user?Ln().catch(e=>console.error("[feed] load failed:",e)):Mt().catch(e=>console.error("[explore] load failed:",e));setInterval(()=>{ve.getState().user?Ln().catch(()=>{}):Mt().catch(()=>{})},En);
